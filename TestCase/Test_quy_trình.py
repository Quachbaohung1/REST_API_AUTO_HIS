...
from Khám_bệnh_CDDV.POST import process_examination_services
...
class TestProcessPatient(unittest.TestCase):
    ...
    def process_file(self, file_path):
        ...
        with api_semaphore:
            ...
            frVisitEntryIds, all_datas = process_examination_services(examination_data, testcase_CDDV, entry_ids)
            # Hàm trả kết quả CLS
            ...
            # result_apis = process_prescription_services(prescription_data, testcase_KT, entry_ids)
        ...
        if patientCodes and entry_ids and frVisitEntryIds and VPDatas:  # Kiểm tra nếu patientCodes và entry_ids không trống
            self.entry_ids[file_path] = entry_ids
            self.patientCodes[file_path] = patientCodes
            self.frVisitEntryIds[file_path] = frVisitEntryIds
            # self.result_apis[file_path] = result_apis
